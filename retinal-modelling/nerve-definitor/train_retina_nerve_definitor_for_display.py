        ...
        for fold, (train_idx, val_idx) in enums:
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    '''inputs = tochfunc.interpolate(inputs,
                                                      size=tuple(
                                                          [int(config.img_shapes[0] / 2),
                                                           int(config.img_shapes[1] / 2)]),
                                                      mode='bilinear', align_corners=False)
                    targets = tochfunc.interpolate(targets,
                                                     size=tuple(
                                                         [int(config.img_shapes[0] / 2),
                                                          int(config.img_shapes[1] / 2)]),
                                                     mode='bilinear', align_corners=False)'''
                    ...
                    for loss_name, loss_value in loss:
                        '''if not total_eval_loss.keys().__contains__(loss_name):
                            total_eval_loss[loss_name] = 0'''
            ...
            if config.save_cp_backup_iter \
                ...
                misc.save_nerve_definitor(f"states_{global_epoch + 1}.pth", nerve_definitor_pass, optimizer, global_epoch,
                                          config)
